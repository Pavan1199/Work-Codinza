...
var qw = mongojs('mongodb://pa1:pa1@cluster0-shard-00-00-jlsfv.mongodb.net:27017/sl11?ssl=true&replicaSet=Cluster0-shard-0&authSource=admin', ['bb'])
...
api.on('message', function(message){
    ...
    if(msg == 'hi' || msg == 'hello' || msg == 'hey' ){
    	...
    	if(str[1] == 'ase'){
    		api.sendMessage({
			chat_id: message.chat.id,
			text: 'Exam Time Table ASE' 		
			});
    	}
    	else{
    		api.sendMessage({
			chat_id: message.chat.id,
			text: 'Exam Time Table ASAS' 		
			});
    	}
    ...
    else if(msg.includes('time table')){
    	api.sendMessage({
		chat_id: message.chat.id,
		text: 'Time Table Batch Sem' 		
		}); 	
